...
const dbToDeal = (db: any): Deal => ({
  ...
  implantationValue: Number(db.implantationValue ?? db.implantation_value ?? 0),
  monthlyValue: Number(db.monthlyValue ?? db.monthly_value ?? 0),
  ...
  commissionRateSnapshot: db.commissionRateSnapshot != null ? Number(db.commissionRateSnapshot) : (db.commission_rate_snapshot != null ? Number(db.commission_rate_snapshot) : undefined),
  commissionAmountSnapshot: db.commissionAmountSnapshot != null ? Number(db.commissionAmountSnapshot) : (db.commission_amount_snapshot != null ? Number(db.commission_amount_snapshot) : undefined),
  isTestData: db.isTestData ?? db.is_test_data,
  isInstallment: db.isInstallment ?? db.is_installment ?? false,
  installmentCount: Number(db.installmentCount ?? db.installment_count ?? 0),
